...
const ProfessionalA2Form = () => {
  const [formData, setFormData] = useState({
    ann_title: 'Policy Dissemination & Acknowledgment Form',
    ann_no: 'A2',
    ann_sop: 'SOP A.2 – Policy Communication & Tracking',
    ann_bylaws: 'Section 3 – Policy Implementation',
    ann_version: '1.0',
    ann_effective: '',
    ann_cust: 'Human Resources',
    ann_auth: 'HR Manager / Department Heads',
    ann_purpose: 'This Annexure provides the standardized Policy Dissemination & Acknowledgment Form for Koyili Hospital. It ensures proper communication and acknowledgment of HR policies by all employees.',
    policy_title: '',
    policy_version: '',
    policy_date: '',
    emp_name: '',
  ...
  const handleSave = () => {
    ...
    a.href = url;
    a.download = 'Annexure_A2_Policy_Acknowledgment.json';
    a.click();
  ...
  const handleReset = () => {
    if (window.confirm('Reset all fields to default values?')) {
      setFormData({
        ...
        ann_cust: 'Human Resources',
        ann_auth: 'HR Manager / Department Heads',
        ann_purpose: 'This Annexure provides the standardized Policy Dissemination & Acknowledgment Form.',
        policy_title: '',
        policy_version: '',
        policy_date: '',
        emp_name: '',
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-between gap-6">
          ...
          <div className="flex-1 text-center">
            ...
            <div className="mt-3 pt-3 border-t border-blue-400/30">
              <h2 className="text-xl font-bold">POLICY ACKNOWLEDGMENT</h2>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><FileText className="w-5 h-5" />Policy Details</h3>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Version</label><input type="text" name="policy_version" value={formData.policy_version} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Effective Date</label><input type="date" name="policy_date" value={formData.policy_date} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Acknowledgment Details</h3>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Acknowledgment Date</label><input type="date" name="ack_date" value={formData.ack_date} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Communication Method</label><select name="ack_method" value={formData.ack_method} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg"><option value="">Select method</option><option value="email">Email</option><option value="portal">Employee Portal</option><option value="training">Training Session</option><option value="meeting">Department Meeting</option></select></div>
            </div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Training/Briefing Date (if applicable)</label><input type="date" name="training_date" value={formData.training_date} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-700">I hereby acknowledge that I have received, read, and understood the above-mentioned policy. I commit to comply with all provisions outlined in the policy.</p>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Employee Signature</label><input type="text" name="emp_signature" value={formData.emp_signature} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Date</label><input type="date" name="emp_sig_date" value={formData.emp_sig_date} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 px-8 py-5 mt-8">
        <div className="flex items-center justify-between text-white">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-blue-200">Policy Acknowledgment</p>
          </div>
        </div>
      </div>
    </div>
