...
const ProfessionalB1Form = () => {
  const [formData, setFormData] = useState({
    req_dept: '', req_position: '', req_vacancies: '', req_type: 'Full-time',
    req_reason: '', req_resp: '', req_qual: '', req_exp: '', req_skills: '',
    hod_name: '', hod_date: '', hr_name: '', hr_date: '', ceo_name: '', ceo_date: ''
  });
  ...
  const handleSave = () => {
    ...
    a.href = url;
    a.download = 'Annexure_B1_Manpower_Requisition.json';
    a.click();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><Users className="w-5 h-5" />Position Details</h3>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Position Title</label><input type="text" name="req_position" value={formData.req_position} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Number of Vacancies</label><input type="number" name="req_vacancies" value={formData.req_vacancies} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Employment Type</label><select name="req_type" value={formData.req_type} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg"><option value="Full-time">Full-time</option><option value="Part-time">Part-time</option><option value="Contract">Contract</option><option value="Temporary">Temporary</option></select></div>
          </div>
        </div>

        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Justification & Requirements</h3>
          <div className="space-y-4">
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Reason for Requisition</label><textarea name="req_reason" value={formData.req_reason} onChange={handleChange} rows="3" className="w-full px-3 py-2 border border-gray-300 rounded-lg resize-none" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Key Responsibilities</label><textarea name="req_resp" value={formData.req_resp} onChange={handleChange} rows="3" className="w-full px-3 py-2 border border-gray-300 rounded-lg resize-none" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Required Qualifications</label><textarea name="req_qual" value={formData.req_qual} onChange={handleChange} rows="2" className="w-full px-3 py-2 border border-gray-300 rounded-lg resize-none" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Experience Required</label><input type="text" name="req_exp" value={formData.req_exp} onChange={handleChange} placeholder="e.g., 2-3 years" className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Required Skills</label><textarea name="req_skills" value={formData.req_skills} onChange={handleChange} rows="2" className="w-full px-3 py-2 border border-gray-300 rounded-lg resize-none" /></div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 px-8 py-5 mt-8">
        <div className="flex items-center justify-between text-white">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-blue-200">Manpower Requisition Form</p>
          </div>
        </div>
      </div>
    </div>
