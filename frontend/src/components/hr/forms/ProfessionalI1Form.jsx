...
import { Save, Upload, RotateCcw, Printer, Download, Plus, Trash2, DollarSign, Calculator } from 'lucide-react';

const ProfessionalI1Form = () => {
  const [formData, setFormData] = useState({
    period_from: '', period_to: '', decl_name: '', prep_name: '', prep_date: '',
    ver_name: '', ver_date: '', fin_name: '', fin_date: '', dir_name: '', dir_date: ''
  });

  const [payrollRows, setPayrollRows] = useState([
    { emp_id: '', emp_name: '', dept: '', desig: '', days: '', lop: '', ot: '', allow: '', inc: '', bonus: '', ded: '', net: '', remarks: '' }
  ]);
  ...
  const handleRowChange = (index, field, value) => {
    ...
    // Auto-calculate net if financial fields change
    if (['allow', 'inc', 'bonus', 'ded'].includes(field)) {
      const allow = parseFloat(updated[index].allow) || 0;
      const inc = parseFloat(updated[index].inc) || 0;
      const bonus = parseFloat(updated[index].bonus) || 0;
      const ded = parseFloat(updated[index].ded) || 0;
      updated[index].net = (allow + inc + bonus - ded).toFixed(2);
    }
    
    setPayrollRows(updated);
  };

  const addRow = () => setPayrollRows([...payrollRows, { emp_id: '', emp_name: '', dept: '', desig: '', days: '', lop: '', ot: '', allow: '', inc: '', bonus: '', ded: '', net: '', remarks: '' }]);
  const removeRow = (index) => payrollRows.length > 1 && setPayrollRows(payrollRows.filter((_, i) => i !== index));

  const calculateAll = () => {
    const updated = payrollRows.map(row => ({
      ...row,
      net: ((parseFloat(row.allow) || 0) + (parseFloat(row.inc) || 0) + (parseFloat(row.bonus) || 0) - (parseFloat(row.ded) || 0)).toFixed(2)
    }));
    setPayrollRows(updated);
  };
  ...
  const handleSave = () => {
    const data = { logo: logoData, formData, payrollRows };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'Annexure_I1_Payroll_Input.json';
    a.click();
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        try {
          ...
          if (data.payrollRows) setPayrollRows(data.payrollRows);
        } catch (err) {
  ...
  const handleReset = () => {
    if (window.confirm('Reset all fields?')) {
      ...
      setPayrollRows([{ emp_id: '', emp_name: '', dept: '', desig: '', days: '', lop: '', ot: '', allow: '', inc: '', bonus: '', ded: '', net: '', remarks: '' }]);
  ...
  const totalNet = payrollRows.reduce((sum, row) => sum + (parseFloat(row.net) || 0), 0);
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex items-center justify-between gap-6">
          ...
          <div className="flex-1 text-center">
            ...
            <div className="mt-3 pt-3 border-t border-blue-400/30">
              <h2 className="text-xl font-bold">PAYROLL INPUT SHEET</h2>
            </div>
          </div>
          <div className="flex-shrink-0 text-right space-y-2">
            <div className="bg-white/10 backdrop-blur-sm px-4 py-2 rounded-lg border border-white/20">
              <p className="text-xs text-blue-200 mb-0.5">Annexure</p>
              <p className="text-2xl font-bold">I1</p>
            </div>
      ...
      <div className="max-w-[95%] mx-auto px-6 py-8">
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Payroll Period</h3>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2"><DollarSign className="w-5 h-5" />Payroll Data ({payrollRows.length} employees)</h3>
              <p className="text-sm text-gray-600">Total Net Payable: ₹{totalNet.toFixed(2)}</p>
            </div>
            ...
            <table className="w-full border-collapse text-xs">
              <thead>
                <tr className="bg-blue-50">
                  ...
                  <th className="border border-gray-200 px-1 py-2">Days</th>
                  <th className="border border-gray-200 px-1 py-2">LOP</th>
                  <th className="border border-gray-200 px-1 py-2">OT(Hrs)</th>
                  <th className="border border-gray-200 px-1 py-2">Allow(₹)</th>
                  <th className="border border-gray-200 px-1 py-2">Inc(₹)</th>
                  <th className="border border-gray-200 px-1 py-2">Bonus(₹)</th>
                  <th className="border border-gray-200 px-1 py-2">Ded(₹)</th>
                  <th className="border border-gray-200 px-1 py-2 bg-green-50">Net(₹)</th>
              ...
              <tbody>
                {payrollRows.map((row, index) => (
                  <tr key={index}>
                    ...
                    <td className="border border-gray-200 px-1 py-1"><input type="number" value={row.days} onChange={(e) => handleRowChange(index, 'days', e.target.value)} className="w-12 px-1 py-0.5 border border-gray-300 rounded text-xs" /></td>
                    <td className="border border-gray-200 px-1 py-1"><input type="number" value={row.lop} onChange={(e) => handleRowChange(index, 'lop', e.target.value)} className="w-12 px-1 py-0.5 border border-gray-300 rounded text-xs" /></td>
                    <td className="border border-gray-200 px-1 py-1"><input type="number" step="0.1" value={row.ot} onChange={(e) => handleRowChange(index, 'ot', e.target.value)} className="w-14 px-1 py-0.5 border border-gray-300 rounded text-xs" /></td>
                    <td className="border border-gray-200 px-1 py-1"><input type="number" step="0.01" value={row.allow} onChange={(e) => handleRowChange(index, 'allow', e.target.value)} className="w-20 px-1 py-0.5 border border-gray-300 rounded text-xs" /></td>
                    <td className="border border-gray-200 px-1 py-1"><input type="number" step="0.01" value={row.inc} onChange={(e) => handleRowChange(index, 'inc', e.target.value)} className="w-20 px-1 py-0.5 border border-gray-300 rounded text-xs" /></td>
                    <td className="border border-gray-200 px-1 py-1"><input type="number" step="0.01" value={row.bonus} onChange={(e) => handleRowChange(index, 'bonus', e.target.value)} className="w-20 px-1 py-0.5 border border-gray-300 rounded text-xs" /></td>
                    <td className="border border-gray-200 px-1 py-1"><input type="number" step="0.01" value={row.ded} onChange={(e) => handleRowChange(index, 'ded', e.target.value)} className="w-20 px-1 py-0.5 border border-gray-300 rounded text-xs" /></td>
                    <td className="border border-gray-200 px-1 py-1 bg-green-50"><input value={row.net} readOnly className="w-24 px-1 py-0.5 bg-green-50 border border-gray-300 rounded text-xs font-semibold" /></td>
                    ...
                    <td className="border border-gray-200 px-1 py-1 text-center">
                      <button onClick={() => removeRow(index)} disabled={payrollRows.length === 1} className="p-0.5 text-red-600 hover:text-red-800 disabled:text-gray-400">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Signatures & Verification</h3>
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              ...
              <tr>
                <td className="border border-gray-200 px-3 py-2">Verified By (HR)</td>
                <td className="border border-gray-200 px-3 py-2"><input name="ver_name" value={formData.ver_name} onChange={handleChange} className="w-full px-2 py-1 border border-gray-300 rounded" /></td>
                <td className="border border-gray-200 px-3 py-2"><input type="date" name="ver_date" value={formData.ver_date} onChange={handleChange} className="w-full px-2 py-1 border border-gray-300 rounded" /></td>
              </tr>
              <tr>
                <td className="border border-gray-200 px-3 py-2">Approved By (Finance)</td>
              ...
              <tr>
                <td className="border border-gray-200 px-3 py-2">Final Approval (Director)</td>
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 px-8 py-5 mt-8">
        <div className="flex items-center justify-between text-white">
          ...
          <div className="text-right">
            <p className="text-sm font-bold">Form I1</p>
            <p className="text-xs text-blue-200">Payroll Input Sheet</p>
          </div>
        </div>
      </div>
    </div>
...
export default ProfessionalI1Form;
