...
const ProfessionalK3Form = () => {
  const [formData, setFormData] = useState({
    effectiveDate: '', hrName: '', hrDate: ''
  });

  const [deviceRecords, setDeviceRecords] = useState([
    { empId: '', empName: '', dept: '', deviceType: '', serialNo: '', allocDate: '', returnDate: '', condition: '', remarks: '' }
  ]);
  ...
  const handleRecordChange = (index, field, value) => {
    const updated = [...deviceRecords];
    updated[index][field] = value;
    setDeviceRecords(updated);
  };

  const addRow = () => {
    setDeviceRecords([...deviceRecords, { empId: '', empName: '', dept: '', deviceType: '', serialNo: '', allocDate: '', returnDate: '', condition: '', remarks: '' }]);
  };

  const removeRow = (index) => {
    if (deviceRecords.length > 1) {
      setDeviceRecords(deviceRecords.filter((_, i) => i !== index));
    }
  };
  ...
  const handleSave = () => {
    const data = { logo: logoData, formData, deviceRecords };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'Annexure_K3_Device_Register.json';
    a.click();
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        try {
          ...
          if (data.deviceRecords) setDeviceRecords(data.deviceRecords);
        } catch (err) {
  ...
  const handleReset = () => {
    if (window.confirm('Reset all fields?')) {
      setFormData({ effectiveDate: '', hrName: '', hrDate: '' });
      setDeviceRecords([{ empId: '', empName: '', dept: '', deviceType: '', serialNo: '', allocDate: '', returnDate: '', condition: '', remarks: '' }]);
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="max-w-7xl mx-auto bg-white shadow-lg">
        <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 px-8 py-6">
          <div className="grid grid-cols-[200px_1fr_200px] gap-6 items-center">
            ...
            <div className="text-center text-white">
              ...
              <p className="text-sm opacity-90 mb-1">IT Department • Asset Management</p>
              <p className="text-xs opacity-75">NABH Accredited • Device Tracking</p>
            </div>
            <div className="text-right space-y-1">
              <div className="inline-block bg-white/20 backdrop-blur-sm px-3 py-1 rounded text-xs text-white font-semibold">Annexure K3</div>
        ...
        <div className="bg-blue-50 border-b-2 border-blue-900 px-8 py-4">
          <h2 className="text-xl font-bold text-blue-900 text-center">DEVICE ALLOCATION AND RETURN REGISTER</h2>
        </div>
          ...
          <div className="border border-gray-300 rounded-lg overflow-hidden">
            <div className="bg-blue-50 px-4 py-2 border-b border-gray-300 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-blue-900">Device Allocation Register</h3>
              </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="border border-gray-300 px-2 py-2 font-semibold text-blue-900">Device Type</th>
                    <th className="border border-gray-300 px-2 py-2 font-semibold text-blue-900">Serial No.</th>
                    <th className="border border-gray-300 px-2 py-2 font-semibold text-blue-900">Alloc Date</th>
                    <th className="border border-gray-300 px-2 py-2 font-semibold text-blue-900">Return Date</th>
                    <th className="border border-gray-300 px-2 py-2 font-semibold text-blue-900">Condition</th>
                ...
                <tbody>
                  {deviceRecords.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td className="border border-gray-300 px-2 py-1">
                        <input type="text" value={row.deviceType} onChange={(e) => handleRecordChange(index, 'deviceType', e.target.value)} placeholder="Laptop/Phone/Tablet" className="w-full border-gray-300 rounded px-1 py-1 text-xs" />
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        <input type="text" value={row.serialNo} onChange={(e) => handleRecordChange(index, 'serialNo', e.target.value)} className="w-full border-gray-300 rounded px-1 py-1 text-xs" />
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        <input type="date" value={row.allocDate} onChange={(e) => handleRecordChange(index, 'allocDate', e.target.value)} className="w-full border-gray-300 rounded px-1 py-1 text-xs" />
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        <input type="date" value={row.returnDate} onChange={(e) => handleRecordChange(index, 'returnDate', e.target.value)} className="w-full border-gray-300 rounded px-1 py-1 text-xs" />
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                        <input type="text" value={row.condition} onChange={(e) => handleRecordChange(index, 'condition', e.target.value)} placeholder="Good/Fair/Damaged" className="w-full border-gray-300 rounded px-1 py-1 text-xs" />
                      </td>
                      <td className="border border-gray-300 px-2 py-1">
                      ...
                      <td className="border border-gray-300 px-2 py-1 text-center print:hidden">
                        <button onClick={() => removeRow(index)} className="text-red-600 hover:text-red-800" disabled={deviceRecords.length === 1}>
                          <Trash2 size={14} />
                        </button>
                      </td>
                    </tr>
        ...
        <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 px-8 py-4 mt-8">
          <div className="flex justify-between items-center text-white text-xs">
            ...
            <div className="font-semibold">Annexure K3 • Device Allocation and Return Register</div>
          </div>
        </div>
      </div>
    </div>
...
export default ProfessionalK3Form;