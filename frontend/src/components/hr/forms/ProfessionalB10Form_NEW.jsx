...
const ProfessionalB10Form = () => {
  const [formData, setFormData] = useState({
    emp_name: '', position: '', reporting_to: '', dept: '', compensation: '',
    start_date: '', probation: '6 months', termination_notice: '30 days',
    issuer_name: '', issuer_desig: '', emp_ack_name: '', emp_ack_date: ''
  });
  ...
  const removeDuty = (index) => setDuties(duties.filter((_, i) => i !== index));
  ...
  const handleSave = () => {
    const data = { logo: logoData, formData, duties };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'Annexure_B10_Appointment_Letter.json';
    a.click();
  ...
  const handleReset = () => {
    if (window.confirm('Reset all fields?')) {
      ...
      setDuties(['Perform duties as assigned by the supervisor', 'Maintain professional conduct and ethics', 'Adhere to hospital policies and procedures']);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><FileText className="w-5 h-5" />Appointment Details</h3>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div><label className="block text-sm font-semibold text-gray-700 mb-1">Termination Notice</label><input type="text" name="termination_notice" value={formData.termination_notice} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-900">Duties & Responsibilities</h3>
            <button onClick={addDuty} className="px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center gap-2 text-sm font-semibold">
          ...
          <div className="space-y-3">
            {duties.map((duty, index) => (
              <div key={index} className="flex gap-2">
                <input value={duty} onChange={(e) => updateDuty(index, e.target.value)} className="flex-1 px-3 py-2 border border-gray-300 rounded-lg" />
                <button onClick={() => removeDuty(index)} className="px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 mb-5">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Employee Acknowledgment</h3>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Name</label><input type="text" name="emp_ack_name" value={formData.emp_ack_name} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Date</label><input type="date" name="emp_ack_date" value={formData.emp_ack_date} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 px-8 py-5 mt-8">
        <div className="flex items-center justify-between text-white">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-blue-200">Appointment Letter</p>
          </div>
        </div>
      </div>
    </div>
